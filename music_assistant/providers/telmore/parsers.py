"""Parsers for Telmore Musik API responses."""
...
async def parse_track(provider: TelmoreMusikProvider, track_obj: JsonLike) -> Track:
    """Parse track data from Telmore API response."""
    track = Track(
...
def parse_artist(provider: TelmoreMusikProvider, artist_obj: JsonLike) -> Artist:
    """Parse artist data from Telmore API response."""
    artist = Artist(
...
async def parse_album(provider: TelmoreMusikProvider, album_obj: JsonLike) -> Album:
    """Parse album data from Telmore API response."""
...
async def parse_playlist(provider: TelmoreMusikProvider, playlist_obj: JsonLike) -> Playlist:
    """Parse playlist data from Telmore API response."""
    playlist = Playlist(
...
async def parse_lyrics(lyrics: list[JsonLike]) -> tuple[str | None, str | None]:
    """Parse the Telmore lyrics payload and extract the lyric text in two formats if possible.
